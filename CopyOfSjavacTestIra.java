 ...
 * @author Nir&Ira
 */
public class CopyOfSjavacTestIra extends TestCase {

	private static final String TESTS_PATH = "C:\\Users\\t6631696\\Dropbox\\תלפיות לד\\"
			+ "אקדמיה\\קורסים סמסטר ב\\CLIDS\\תרגילים\\תרגיל 4\\ex4_files";

	public CopyOfSjavacTestIra() {
	}
...
//	}
	...
	public void testFile(int number, int exitValue) {
		try {
			...
			assertEquals("Wrong exit status in file No. " + number, exitValue,
					e.status);
			// assertEquals("Wrong exit status in file No. " + number, 0,
			// e.status);
		}
	}
